defmodule Pixir.WorkflowsTest do
  ...
  test "apply_from validation rejects in dry_run and run", %{sid: sid, ws: ws} do
    ...
    # Each negative spec must fail for ITS OWN reason (CodeRabbit round on
    # PR #287): expected {location, message-fragment} per case, in order.
    expectations = [
      {"/steps/1/apply_from", "must reference a previous virtual_overlay step"},
      {"/steps/2/apply_from", "must use workspace_mode virtual_overlay"},
      {"/steps/2/apply_from", "must be listed in depends_on"},
      {"/steps/2/apply_from", "requires write_set"},
      {"/steps/2/apply_from", "do not spawn subagents"}
    ]
  ...
  test "bounded_write rejects writer posture read-only agents but allows read-only posture", %{
    ...
    assert details["location"] == "/steps/1/agent"
