...
| attached `pixir delegate --spec ... --json` | `5` | Timeout, backpressure, unavailable manager, or daemon requirement. | Inspect timeout/backpressure details; retry with adjusted budgets or daemon setup. |
| attached `pixir delegate --spec ... --json` | `6` | Domain work reached an incomplete terminal state such as `partial`, `timed_out`, `failed`, or `cancelled`. | Parse the envelope; inspect `children[*].status`, workflow buckets, `write_destination`, and diagnostics before deciding whether the result is usable. |
